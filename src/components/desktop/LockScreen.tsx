...
export const LockScreen: React.FC<LockScreenProps> = ({ onUnlock }) => {
  ...
  return (
    ...
    >
      {/* Shared layered backdrop — loads instantly via blur placeholder + preload */}
      <DesktopBackground intensity={26} showOrbs={false}>
        {/* Extra darkening so the lock UI stays readable */}
        <div className="absolute inset-0 bg-purple-950/55 mix-blend-multiply pointer-events-none" />
        <div className="absolute inset-0 bg-black/30 pointer-events-none" />
      </DesktopBackground>

      {/* Diagonal slice decoration */}
      <div className="absolute top-0 right-0 w-2/3 h-full bg-black/40 -skew-x-12 translate-x-1/4 z-0 border-l-4 border-white/20 pointer-events-none" />
      ...
      >
        <div className="relative">
          {/* layered plate behind the clock */}
          <div className="absolute inset-0 translate-x-3 translate-y-3 bg-purple-600 border-4 border-black" />
          <div className="relative bg-black px-8 py-4 transform -rotate-2 border-4 border-white shadow-[10px_10px_0px_0px_rgba(0,0,0,0.6)]">
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-2xl font-p5 text-white tracking-[0.15em] uppercase [word-spacing:0.5rem]">Take Your Time</span>
